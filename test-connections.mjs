...
async function testMongoDB() {
  ...
  try {
    ...
    log(`   Connecting to: ${mongoUri.split('@')[1] || 'MongoDB'}`, colors.yellow);
    ...
    if (error.message.includes('Authentication failed')) {
      ...
      log('   Current password contains @ which may need encoding', colors.yellow);
    } else if (error.message.includes('ENOTFOUND')) {
      log('\n💡 Tip: Check your MongoDB cluster URL', colors.yellow);
      log('   Make sure the cluster is not paused', colors.yellow);
    } else if (error.message.includes('IP')) {
      log('\n💡 Tip: Add your IP address to MongoDB Atlas whitelist', colors.yellow);
    }
...
async function testPinecone() {
  ...
  try {
    ...
    log(`   Available indexes: ${indexes.indexes?.map(i => i.name).join(', ') || 'None'}`, colors.green);
    ...
    if (indexExists) {
      ...
      log('   You need to create this index in Pinecone dashboard', colors.yellow);
...
async function runAllTests() {
  ...
  log(`Pinecone: ${results.pinecone ? '✅ Connected' : '❌ Failed'}`, results.pinecone ? colors.green : colors.red);
  ...
  if (allPassed) {
    ...
    log('2. Pinecone: Create index "learnbyai-content" in dashboard', colors.yellow);
